...
exports.addProgramToTopic = async (req, res) => {
  ...
  try {
    ...
    if (!topic || !program) {
      return res.status(404).json({ success: false, message: "Materia o programa no encontrado" });
    }
...
exports.getTopicsByProgram = async (req, res) => {
  ...
  try {
    const topics = await Topic.find({ programs: programId }).populate("programs", "name");
...
exports.assignProgramsToAllTopics = async (req, res) => {
  ...
  try {
    ...
    if (programs.length !== programIds.length) {
      return res.status(404).json({ success: false, message: "Uno o más programas no encontrados" });
    }
    ...
    res.status(200).json({ success: true, message: "Programas asignados a todas las materias existentes" });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
...
const topicNames = [
  "Matemáticas Básicas", "Física I", "Química General", "Biología",
  "Historia Universal", "Geografía", "Literatura", "Lengua Española",
  "Cálculo Diferencial", "Programación I", "Ingeniería de Software", "Redes",
  "Algoritmos y Estructuras de Datos", "Sistemas Operativos", "Bases de Datos",
  "Inteligencia Artificial", "Arquitectura de Computadores", "Ética Profesional",
  "Desarrollo Web", "Administración de Proyectos", "Cálculo Integral",
  "Estadística", "Física II", "Química Orgánica", "Microbiología",
  "Historia de Colombia", "Geografía Física", "Literatura Hispanoamericana",
  "Lengua Extranjera", "Programación II", "Ingeniería de Requisitos",
  "Seguridad Informática", "Redes Avanzadas", "Optimización",
  "Minería de Datos", "Arquitectura de Software", "Derecho Informático",
  "Desarrollo Móvil", "Gestión de Calidad"
];
...
exports.assignTopicsToPrograms = async (req, res) => {
  try {
    const excludedPrograms = ["66836135323b7c660d055f95", "66836135323b7c660d055ff0"];
    ...
    programs.forEach((program, programIndex) => {
      for (let i = 0; i < 20; i++) {
        const topicName = topicNames[(programIndex * 20 + i) % topicNames.length]; // Asignar nombres de materias cíclicamente
    ...
    res.status(200).json({ success: true, message: "Materias asignadas a los programas exitosamente" });
